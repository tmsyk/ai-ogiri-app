          ...
          <>
            ...
            {gamePhase === 'result' && (
              <div className="animate-in zoom-in duration-300 pb-20">
                ...
                <div className="p-8 text-center bg-gradient-to-b from-white to-slate-50"><p className="text-sm text-slate-400 font-bold mb-2">ベストアンサー</p><p className="text-3xl md:text-4xl font-black text-indigo-600 leading-tight mb-6">{selectedSubmission?.answerText}</p>
            ...
            {gamePhase === 'final_result' && (
              <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-500 text-slate-900">
                ...
                <h2 className="text-xl font-bold text-slate-500 mb-2">{gameConfig.mode === 'multi' ? '🏆 優勝決定！' : (gameConfig.singleMode === 'score_attack' ? `全${TOTAL_ROUNDS_SCORE_ATTACK}回戦 終了！` : gameConfig.singleMode === 'survival' ? "GAME OVER..." : "GOAL!!")}</h2>
                <div className="text-6xl font-black text-indigo-600 mb-4">{gameConfig.mode === 'single' && gameConfig.singleMode === 'time_attack' && (startTime && finishTime) ? formatTime(finishTime - startTime) : `${gameConfig.mode === 'multi' ? players.find(p => p.score >= WINNING_SCORE_MULTI)?.name || '???' : players[0].score + (gameConfig.singleMode === 'time_attack' ? '点' : '点')}`}</div>
                {gameConfig.mode === 'single' && (<div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 w-full max-w-sm mb-4"><p className="text-xl font-bold text-slate-800">{gameConfig.singleMode === 'score_attack' && `称号：${players[0].score >= 450 ? "お笑い神" : players[0].score >= 400 ? "大御所" : players[0].score >= 300 ? "真打ち" : "見習い"}`}{gameConfig.singleMode === 'survival' && `スコア: ${players[0].score}点`}{gameConfig.singleMode === 'time_attack' && `合計スコア: ${players[0].score}点`}</p></div>)}
                {gameConfig.mode === 'multi' && (<div className="w-full max-w-sm mb-8 bg-white rounded-xl shadow p-4"><h3 className="font-bold text-slate-600 mb-2 border-b pb-2">最終結果</h3>{players.sort((a,b)=>b.score-a.score).map((p,i)=>(<div key={i} className="flex justify-between py-1 border-b border-slate-100 last:border-0"><span>#{i+1} {p.name}</span><span className="font-bold">{p.score}点</span></div>))}</div>)}
                {!gameConfig.mode === 'multi' && <div className="w-full max-w-sm mb-8"><RankingList mode={gameConfig.singleMode} data={rankings[gameConfig.singleMode]} unit={gameConfig.singleMode==='time_attack'?'':'点'} /></div>}
