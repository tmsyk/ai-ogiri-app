...
// --- バージョン情報 ---
const APP_VERSION = "Ver 0.08";
const UPDATE_LOGS = [
  { version: "Ver 0.08", date: "2026/01/20", content: ["タイムアタックのランキング表示バグを修正", "マルチプレイの優勝判定タイミングを改善"] },
  { version: "Ver 0.07", date: "2026/01/20", content: ["UIの反応速度を改善", "タイマー終了時の不具合を修正", "効果音処理の軽量化"] },
    ...
    <div className="bg-white rounded-3xl p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-2xl relative">
      ...
      {type === 'rule' && (
        <div className="space-y-6 text-slate-700">
          ...
          <section><h4 className="font-bold text-lg mb-2 flex items-center gap-2 border-b pb-1"><Users className="w-5 h-5 text-amber-500" /> みんなで遊ぶ（2人～）</h4><ul className="list-disc list-inside text-sm space-y-1 text-slate-600 ml-1"><li>親と子に分かれて対戦。</li><li>審査時に「ダミー回答」が混ざります。</li><li>親がダミーを選ぶと減点！</li><li>10点先取で優勝！</li></ul></section>
        </div>
      )}
      {type === 'update' && (
        <div className="space-y-6 text-slate-700">
...
export default function AiOgiriApp() {
  ...
  const fetchAiCards = async (count = 10) => {
    const prompt = `大喜利の回答カード（単語・短いフレーズ）を${count}個作成してください。条件: 1.名詞または体言止め。2.具体的で情景が浮かぶ言葉。3.ジャンルはバラバラに。4.既存のありふれた回答は避ける。5.出力はJSON形式。`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-24 text-slate-900">
      ...
      <main className="max-w-2xl mx-auto p-4">
        ...
        {/* Setup Screen */}
        {appMode === 'setup' && (
           <div className="flex flex-col items-center justify-center py-6 animate-in slide-in-from-right">
             ...
             <div className="w-full max-w-md space-y-6 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
               <div className="mb-4"><label className="block text-sm font-bold text-slate-700 mb-2">プレイヤー名</label><div className="relative"><input type="text" value={userName} onChange={(e) => saveUserName(e.target.value)} className="w-full p-3 pl-10 bg-slate-50 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none font-bold" /><User className="w-5 h-5 text-slate-400 absolute left-3 top-3.5" /></div></div>
               {gameConfig.mode === 'single' ? (
                 ...
                 <div><label className="block text-sm font-bold text-slate-700 mb-2">参加人数: {gameConfig.playerCount}人</label><input type="range" min="2" max="10" value={gameConfig.playerCount} onChange={(e) => setGameConfig(prev => ({ ...prev, playerCount: parseInt(e.target.value) }))} className="w-full accent-indigo-600" />
